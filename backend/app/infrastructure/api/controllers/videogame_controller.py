...
@router.post("/", response_model=RegisterVideogameResponse, status_code=201, dependencies=[Depends(require_admin)])
def register_videogame(request: RegisterVideogameRequest, _player_id: int = Depends(get_current_user_id)):
    uow = uow_factory()
...
@router.put("/{videogame_id}", response_model=UpdateVideogameResponse, status_code=200, dependencies=[Depends(require_admin)])
def update_videogame(videogame_id: int, request: UpdateVideogameRequest, _player_id: int = Depends(get_current_user_id)):
    uow = uow_factory()
