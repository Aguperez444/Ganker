...
class TestRegisterCharacterUseCase:
    ...
    def test_register_character_happy_path(self, mock_deps):
        ...
        vg = Videogame(videogame_id=1, name="League of Legends", icon_url="/lol.png", rank_per_role=True)
    ...
    def test_register_character_empty_name_raises_invalid_name(self, mock_deps):
        use_case, _, _ = mock_deps

        with pytest.raises(InvalidCharacterNameException) as exc_info:
            use_case.execute(name="   ", videogame_id=1, icon_file=MagicMock(), icon_filename="ahri.png")

        assert exc_info.value.status_code == 400

    def test_register_character_game_not_found(self, mock_deps):
        ...
        with pytest.raises(VideogameNotFoundException) as exc_info:
            use_case.execute(name="Ahri", videogame_id=999, icon_file=MagicMock(), icon_filename="ahri.png")

        assert exc_info.value.status_code == 404

    def test_register_character_duplicate_name_in_same_game(self, mock_deps):
        use_case, uow, _ = mock_deps
    ...
    def test_register_character_missing_file_raises_exception(self, mock_deps):
        use_case, uow, _ = mock_deps
        vg = Videogame(videogame_id=1, name="LoL", icon_url="/icon.png", rank_per_role=True)
        uow.videogame_repo.get_videogame_by_id.return_value = vg
        uow.character_repo.get_character_by_name_and_videogame.return_value = None
        ...
        with pytest.raises(FileNotNullException) as exc_info:
            use_case.execute(name="Ahri", videogame_id=1, icon_file=None, icon_filename="ahri.png")

        assert exc_info.value.status_code == 400

    def test_register_character_missing_filename_raises_exception(self, mock_deps):
        use_case, uow, _ = mock_deps
        vg = Videogame(videogame_id=1, name="LoL", icon_url="/icon.png", rank_per_role=True)
        uow.videogame_repo.get_videogame_by_id.return_value = vg
        uow.character_repo.get_character_by_name_and_videogame.return_value = None
        ...
        with pytest.raises(FileNameNotNullException) as exc_info:
            use_case.execute(name="Ahri", videogame_id=1, icon_file=MagicMock(), icon_filename="")

        assert exc_info.value.status_code == 400

    def test_register_character_db_error_cleans_up_file(self, mock_deps):
        use_case, uow, storage_service = mock_deps
