...
class TestCreateRoleUseCase:
    ...
    def test_create_role_happy_path(self, mock_deps):
        ...
        res = use_case.execute(game_id=1, name="Mid", icon_stream=fake_stream, filename="mid.png")
    ...
    def test_create_role_empty_name(self, mock_deps):
        use_case, _, _ = mock_deps

        with pytest.raises(InvalidRoleNameException) as exc_info:
            use_case.execute(game_id=1, name="  ", icon_stream=MagicMock(), filename="mid.png")

        assert exc_info.value.status_code == 400

    def test_create_role_videogame_not_found(self, mock_deps):
        ...
        with pytest.raises(VideogameNotFoundException) as exc_info:
            use_case.execute(game_id=999, name="Mid", icon_stream=MagicMock(), filename="mid.png")

        assert exc_info.value.status_code == 404

    def test_create_role_duplicate_name(self, mock_deps):
        use_case, uow, _ = mock_deps
    ...
    def test_create_role_missing_stream(self, mock_deps):
        use_case, uow, _ = mock_deps
    ...
    def test_create_role_missing_filename(self, mock_deps):
        use_case, uow, _ = mock_deps
    ...
    def test_create_role_db_error_deletes_file(self, mock_deps):
        use_case, uow, storage_service = mock_deps
