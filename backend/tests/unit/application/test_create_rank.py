...
class TestCreateRankUseCase:
    ...
    def test_create_rank_happy_path(self, mock_deps):
        ...
        result = use_case.execute(game_id=1, name="Gold", icon_stream=MagicMock(), filename="gold.png", value=1000)
    ...
    def test_create_rank_empty_name(self, mock_deps):
        use_case, _, _ = mock_deps

        with pytest.raises(InvalidRankNameException) as exc_info:
            use_case.execute(game_id=1, name="  ", icon_stream=MagicMock(), filename="gold.png", value=1000)

        assert exc_info.value.status_code == 400

    def test_create_rank_negative_value(self, mock_deps):
        use_case, _, _ = mock_deps

        with pytest.raises(InvalidRankValueException) as exc_info:
            use_case.execute(game_id=1, name="Gold", icon_stream=MagicMock(), filename="gold.png", value=-50)

        assert exc_info.value.status_code == 400

    def test_create_rank_game_not_found(self, mock_deps):
        ...
        with pytest.raises(VideogameNotFoundException) as exc_info:
            use_case.execute(game_id=999, name="Gold", icon_stream=MagicMock(), filename="gold.png", value=1000)

        assert exc_info.value.status_code == 404

    def test_create_rank_duplicate_name(self, mock_deps):
        use_case, uow, _ = mock_deps
    ...
    def test_create_rank_duplicate_value(self, mock_deps):
        use_case, uow, _ = mock_deps
    ...
    def test_create_rank_db_error_cleans_up_file(self, mock_deps):
        use_case, uow, storage_service = mock_deps
