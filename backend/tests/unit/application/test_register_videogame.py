...
from app.application.useCase.register_videogame import RegisterVideogame
...
class TestRegisterVideogameUseCase:
    ...
    def mock_deps(self):
        ...
        storage_service.save_image_file = MagicMock(return_value="/media/games/valorant/icon.png")
    ...
    def test_register_videogame_happy_path(self, mock_deps):
        use_case, uow, storage_service = mock_deps

        uow.videogame_repo.get_videogame_by_name.return_value = None
        saved_game = Videogame(videogame_id=1, name="Valorant", icon_url="/media/games/valorant/icon.png", rank_per_role=True)
        uow.videogame_repo.register_videogame.return_value = saved_game

        fake_file = MagicMock()
        result = use_case.execute(name="Valorant", icon_file=fake_file, icon_filename="logo.png", rank_per_role=True)

        assert result.id == 1
        assert result.name == "Valorant"
        assert result.icon_url == "/media/games/valorant/icon.png"
        assert result.rank_per_role is True

        storage_service.save_image_file.assert_called_once()
        uow.videogame_repo.register_videogame.assert_called_once()

    def test_register_videogame_empty_name_raises_invalid_name(self, mock_deps):
        use_case, _, _ = mock_deps

        with pytest.raises(InvalidVideogameNameException) as exc_info:
            use_case.execute(name="   ", icon_file=MagicMock(), icon_filename="icon.png", rank_per_role=True)

        assert exc_info.value.status_code == 400

    def test_register_videogame_already_exists_raises_conflict(self, mock_deps):
        use_case, uow, _ = mock_deps

        existing = Videogame(videogame_id=2, name="Valorant", icon_url="/icon.png", rank_per_role=True)
        ...
        with pytest.raises(VideogameAlreadyExistsException) as exc_info:
            use_case.execute(name="Valorant", icon_file=MagicMock(), icon_filename="icon.png", rank_per_role=True)

        assert exc_info.value.status_code == 409
        assert "Valorant" in exc_info.value.message

    def test_register_videogame_missing_file_raises_exception(self, mock_deps):
        use_case, uow, _ = mock_deps
        uow.videogame_repo.get_videogame_by_name.return_value = None

        with pytest.raises(FileNotNullException) as exc_info:
            use_case.execute(name="Dota 2", icon_file=None, icon_filename="icon.png", rank_per_role=False)

        assert exc_info.value.status_code == 400

    def test_register_videogame_missing_filename_raises_exception(self, mock_deps):
        use_case, uow, _ = mock_deps
        uow.videogame_repo.get_videogame_by_name.return_value = None

        with pytest.raises(FileNameNotNullException) as exc_info:
            use_case.execute(name="Dota 2", icon_file=MagicMock(), icon_filename="", rank_per_role=False)

        assert exc_info.value.status_code == 400

    def test_register_videogame_db_error_deletes_saved_file(self, mock_deps):
        use_case, uow, storage_service = mock_deps
        ...
        with pytest.raises(RuntimeError):
            use_case.execute(name="New Game", icon_file=MagicMock(), icon_filename="icon.png", rank_per_role=False)
