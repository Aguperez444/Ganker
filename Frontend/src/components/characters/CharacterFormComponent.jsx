...
const CharacterForm = ({
  ...
  const handleRemoveIcon = () => {
    ...
    setIconFile(null);
    setPreviewUrl(isEditMode ? resolvedInitialPreview : null);
  ...
  const handleSubmit = async (event) => {
    ...
    if (!trimmedName) {
      setValidationError("El nombre del personaje es obligatorio.");
      return;
    }
    ...
    if (isEditMode && !iconFile && !previewUrl) {
      setValidationError("Debés seleccionar una imagen para el ícono.");
      return;
    }
  ...
  const hasChanges = isEditMode
    ? (name.trim().length > 0 && name.trim() !== initialName.trim()) ||
      String(videogameId) !== String(initialVideogameId) ||
      iconFile !== null
    ...
    <section className="rounded-2xl border border-white/10 bg-ganker-surface p-6 shadow-xl">
      <header className="mb-6">
        <h2 className="font-heading text-xl font-semibold text-ganker-text">
          {isEditMode ? "Modificar personaje" : "Registrar personaje"}
        </h2>

        <p className="mt-1 text-sm text-ganker-muted">
          {isEditMode
            ? "Actualizá la información del personaje."
            : "Agregá un nuevo personaje para un videojuego."}
        </p>
      </header>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-6">
        ...
        <div>
          <label
            ...
            className="mb-2 block text-sm font-medium text-ganker-text"
          >
            Nombre del personaje <span className="text-ganker-orange">*</span>
          </label>
        ...
        <div>
          <label
            htmlFor="character-icon"
            className="mb-2 block text-sm font-medium text-ganker-text"
          >
            Ícono del personaje <span className="text-ganker-orange">*</span>
          </label>
